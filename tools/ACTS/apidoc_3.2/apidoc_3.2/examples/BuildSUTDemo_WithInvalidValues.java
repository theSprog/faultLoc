 ...
 * build a test set to support negative testing with invalid values in domain.
 *
 */
public class BuildSUTDemo_WithInvalidValues {
    public static void main (String[] argv) {
	// build a system configuration
	SUT sut = new SUT ("TCAS_WithInvalidValues");
	...
	cur_vertical_sep.addInvalidValue ("-1"); // add invalid value
	cur_vertical_sep.addInvalidValue ("-2"); // add invalid value
	...
	other_capability.addInvalidValue ("InvalidStr"); // add invalid value
	...
	params.add (climb_inhibit);
	params.add (up_separation);
	Constraint c2 = 
		new Constraint ("Climb_Inhibit = true => Up_Separation > 399",
				params);


	// add these constraints into the sut
	...
	// print into a file
	wrapper.outputInCSVFormat ("tcas_withInvalidValues-output.txt");
   }
}
